...
const SalespersonOrderTimelineDrawer: React.FC<Props> = ({
  ...
  const getEventConfig = (type: string, message: string) => {
    ...
    // Matching iconography from Enterprise v2.0 screenshots
        ...
        <Card className="h-full rounded-none border-none flex flex-col shadow-none overflow-hidden">
          ...
          <CardHeader className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Sales Audit</h3>
          ...
          <CardContent className="flex-1 px-10 py-10 overflow-y-auto relative">
            {loading ? (
              ...
              <div className="relative">
                {/* THE FIX: Vertical Connector Line centered exactly at 20px (half of w-10 node) */}
                  ...
                  {events.map((event) => {
                    ...
                    return (
                      <div key={event.id} className="relative pl-16 group">
                        {/* Status Node (Squarcle) centered on the line */}
